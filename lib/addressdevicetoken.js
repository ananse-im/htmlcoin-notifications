...
AddressDeviceTokenController.prototype.transactionEventHandler = function(txBuffer) {
    ...
    console.log("Prepare sending push notification: ", tx.id);

    var addresses = this.node.services.htmlcoind._getAddressesFromTransaction(tx);

    self.sendNotificationToAddresses(addresses, tx.id, TYPE_TRANSACTION);
...
AddressDeviceTokenController.prototype.blockEventHandler = function(hashBuffer) {
    ...
    self.node.services.htmlcoind.getBlock(hashBuffer.toString('hex'), function (err, block) {
        ...
        for(var i = 0; i < block.transactions.length; i++) {
            ...
            self.sendNotificationToAddresses(addresses, tx.id, TYPE_BLOCK);
...
AddressDeviceTokenController.prototype.sendNotificationToAddresses = function(addresses, txid, type) {
    console.log('Push addresses: ', addresses);
    if (addresses.length == 0) return;
    var self = this;
    this.addressDeviceTokenService.getDevicesTokenByAddresses(addresses, function(deviceTokenRes){
        if (deviceTokenRes == null) return;

        var sentDevices = [];
        // for each devices
        for( var i = 0; i < deviceTokenRes.length; i++) {
            var device = deviceTokenRes[i];
            if (device.isPlayServicesAvailable == null || device.isPlayServicesAvailable == true) {
                var deviceToken = device.deviceToken;
                if (sentDevices.indexOf(deviceToken) > -1) {
                    continue;
                }
                sentDevices.push(deviceToken);
                self.sendFirebaseNotification(deviceToken, device.address, txid, type);
            } else {
                var deviceToken = device.pushyDeviceToken;
                if (sentDevices.indexOf(deviceToken) > -1) {
                    continue;
                }
                sentDevices.push(deviceToken);
                self.sendPushyNotification(deviceToken, device.address, txid, type);
            }
        }
